...
public:
  // конструктор по умолчанию: список элементов пока пуст
    AirportCounter(){
        for(size_t i=0; i<static_cast<uint32_t>(TAirport::Last_); i++){
            numbflight[i].first = static_cast<TAirport>(i);
            numbflight[i].second = 0;
        }
    }
  ...
  AirportCounter(TIterator begin, TIterator end) {
      ...
      for(auto it = begin; it < end; it++){
          numbflight[static_cast<uint32_t>(*it)].second+=1;
      }
  }

  // получить количество элементов, равных данному
  size_t Get(TAirport airport) const {
      return numbflight[static_cast<uint32_t>(airport)].second;
  }

  // добавить данный элемент
  void Insert(TAirport airport) {
      numbflight[static_cast<uint32_t>(airport)].second += 1;
  }
  ...
  void EraseAll(TAirport airport){
      numbflight[static_cast<uint32_t>(airport)].second = 0;
  }
