...
string HttpCodetostring(HttpCode code) {
    switch (code)
        {
            case HttpCode::Ok:
                return "200 OK";
...
public:
    ...
    HttpResponse& SetCode(HttpCode code_){
        code = code_;
        return *this;
    }

  friend ostream& operator << (ostream& output, const HttpResponse& resp) {
      output << "HTTP/1.1 " << HttpCodetostring(resp.code) << endl;
      for (auto& item : resp.headers) {
  ...
  HttpResponse ServeRequest(const HttpRequest& req) {
      ...
      if (req.path == "/add_user") {
        ...
        if (banned_users.count(user_id) == 0) {
          ...
          response_.SetCode(HttpCode::Found);
          response_.AddHeader("Location", "/captcha");
        }
