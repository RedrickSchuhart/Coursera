...
struct AddressHasher {
    size_t operator()(const Address& it) const{
        return (s_hash(it.city)*x*x+s_hash(it.street)*x+it.building);
    }
    size_t x=37;
    hash<string> s_hash;
};
...
struct PersonHasher {
    size_t operator()(const Person& it) const{
        return (it.height*x*x*x+s_hash(it.name)*x*x+d_hash(it.weight)*x+a_hash(it.address));
    }
    size_t x=37;
    hash<string> s_hash;
    hash<double> d_hash;
