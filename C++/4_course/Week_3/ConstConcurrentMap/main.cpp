...
public:
  ...
  size_t GetPart(const K& key_) const {
      size_t key = hasher(key_);
      if(key<0) {
          return (-key)%parts_map.size();
      }
      return key%parts_map.size();
  }

  explicit ConcurrentMap(size_t bucket_count) : parts_map(bucket_count) {}

  WriteAccess operator[](const K& key) {
      size_t nPart = GetPart(key);
      return {lock_guard(parts_map[nPart].m), parts_map[nPart].conteiner[key]} ;
  }

  ReadAccess At(const K& key) const {
      size_t nPart = GetPart(key);
      auto it = parts_map[nPart].conteiner.find(key);
      if(it!=parts_map[nPart].conteiner.end()){
          return {lock_guard(parts_map[nPart].m), (*it).second};
      } else {
  ...
  bool Has(const K& key) const {
      size_t nPart = GetPart(key);
      if(parts_map[nPart].conteiner.find(key)!=parts_map[nPart].conteiner.end()){
          return true;
      } else {
          return false;
      }
  ...
  MapType BuildOrdinaryMap() const {
      ...
      for(auto& part : parts_map) {
          lock_guard<mutex> guard(part.m);
          for(auto&[key, value]:part.conteiner) {
              res[key] = value;
          }
      }
      return res;
